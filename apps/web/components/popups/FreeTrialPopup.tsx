...
export default function FreeTrialPopup() {
  ...
  const isOpen = isPopupOpen('free_trial');
  ...
  useEffect(() => {
    ...
    if (!canShowPopup('free_trial')) {
      return;
    }

    // Check localStorage for 30-day cooldown
    const lastShown = localStorage.getItem('free_trial_last_shown');
    if (lastShown) {
    ...
    const sessionShown = sessionStorage.getItem('free_trial_session_shown');
    if (sessionShown) {
      return;
    }

    // Set up IntersectionObserver at 50% scroll depth
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && !hasTriggered) {
            ...
            trackEvent('free_trial_popup_shown');
            
            // Store in sessionStorage and localStorage
            sessionStorage.setItem('free_trial_session_shown', 'true');
            localStorage.setItem('free_trial_last_shown', Date.now().toString());
          }
        });
      },
      {
    ...
    // Find a target element at 50% of the page
    const targetElement = document.querySelector('[data-scroll-trigger]') || 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Track analytics event
      trackEvent('free_trial_popup_converted', {
        phone: formData.phone,
        source: 'free_trial_popup',
      });
    } catch (err) {
              ...
              {!submitSuccess ? (
                <>
                  ...
                  <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">
                    7 दिन पहले भाव जानें — FREE Trial
                  </h2>
                  <p className="text-gray-600 mb-6">
                    हर सुबह 6:30 AM को WhatsApp पर sell signal मिलेगा। ₹50,000+ loss बचाएं।
                  </p>
                  ...
                  <div className="bg-green-50 rounded-xl p-4 mb-6">
                    <ul className="space-y-2 text-sm text-gray-700">
                      <li className="flex items-center gap-2">
                        ...
                        7-day price predictions (95%+ accuracy)
                      </li>
                      <li className="flex items-center gap-2">
                        <svg className="w-5 h-5 text-green-600 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                          <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                        </svg>
                        Daily WhatsApp alerts at 6:30 AM
                      </li>
                      <li className="flex items-center gap-2">
                        <svg className="w-5 h-5 text-green-600 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                          <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                        </svg>
                        HPAI disease alerts (48-hour warning)
                      </li>
                    </ul>
                  </div>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    >
                      {isSubmitting ? 'जमा हो रहा है...' : '14 दिन FREE Trial शुरू करें →'}
                    </button>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-600 mb-6">
                    आपका 14-day FREE trial शुरू हो गया है। कल सुबह 6:30 AM को पहला signal मिलेगा।
                  </p>
                  <button
                    onClick={handleClose}
