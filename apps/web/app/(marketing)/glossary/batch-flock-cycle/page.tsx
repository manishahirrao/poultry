// FlockIQ — Batch Glossary Page
// File: apps/web/app/(marketing)/glossary/batch-flock-cycle/page.tsx
...
export const metadata: Metadata = {
  title: 'What is Batch (Flock Cycle)? — Poultry Farming Explained',
  description: 'A batch is one complete production cycle from day-old chicks to selling the flock, typically 35–49 days. Learn cost breakdown and optimization.',
  keywords: ['batch meaning poultry', 'flock cycle', 'poultry batch management', 'batch profit calculation', 'बैच झुंड चक्र'],
  openGraph: {
    title: 'What is Batch (Flock Cycle)?',
    description: 'Understand batch in poultry farming - cycle duration, costs, and profit optimization.',
    url: 'https://FlockIQ.ai/glossary/batch-flock-cycle',
  },
  alternates: {
    canonical: 'https://FlockIQ.ai/glossary/batch-flock-cycle',
  },
};

export default function BatchPage() {
  const terms = glossaryData.terms;
  const term = terms.find((t: any) => t.slug === 'batch-flock-cycle');
          ...
          <div className="container mx-auto px-4 max-w-6xl">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <div className="bg-green50 rounded-xl p-6 border border-green200">
                <h3 className="text-lg font-bold text-green900 mb-2">Minimum</h3>
                <p className="text-3xl font-bold text-green700">{term.typicalDuration.minimum}</p>
              </div>
              <div className="bg-brandGreen50 rounded-xl p-6 border border-brandGreen200">
                <h3 className="text-lg font-bold text-brandGreen900 mb-2">Optimal</h3>
                <p className="text-3xl font-bold text-brandGreen700">{term.typicalDuration.optimal}</p>
              </div>
              <div className="bg-amber50 rounded-xl p-6 border border-amber200">
                <h3 className="text-lg font-bold text-amber900 mb-2">Maximum</h3>
                <p className="text-3xl font-bold text-amber700">{term.typicalDuration.maximum}</p>
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="container mx-auto px-4 max-w-6xl">
            ...
            <div className="bg-neutral50 rounded-xl p-8">
              <ul className="space-y-3">
                {Object.entries(term.costBreakdown).map(([key, value]) => (
                  <li key={key} className="flex justify-between items-center py-2 border-b border-neutral200 last:border-0">
                    <span className="text-neutral700 capitalize">{key}</span>
                    <span className="text-brandGreen700 font-semibold">{value}</span>
                  </li>
                ))}
              </ul>
            </div>
      ...
      <section className="py-16 bg-brandGreen700 text-white">
        <div className="container mx-auto px-4 max-w-4xl text-center">
          <h2 className="text-3xl font-bold mb-4">
            Optimize Batch Timing with FlockIQ
          </h2>
          <p className="text-xl text-white/90 mb-8">
            Our platform helps you time your batch sales for maximum profit with 7-day price forecasts.
          </p>
          <Link
            href="/signup"
