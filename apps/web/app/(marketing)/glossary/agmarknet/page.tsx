// FlockIQ — AGMARKNET Glossary Page
// File: apps/web/app/(marketing)/glossary/agmarknet/page.tsx
...
export const metadata: Metadata = {
  title: 'What is AGMARKNET? — Poultry Farming Explained',
  description: 'AGMARKNET is the government portal that publishes daily mandi prices from APMC markets across India. Learn how to use it for historical data.',
  keywords: ['AGMARKNET meaning', 'AGMARKNET portal', 'government mandi prices', 'APMC price portal', 'historical price data'],
  openGraph: {
    title: 'What is AGMARKNET?',
    description: 'Understand AGMARKNET - government portal for daily mandi prices and historical data.',
    url: 'https://FlockIQ.ai/glossary/agmarknet',
  },
  alternates: {
    canonical: 'https://FlockIQ.ai/glossary/agmarknet',
  },
};

export default function AGMARKNETPage() {
  const terms = glossaryData.terms;
  const term = terms.find((t: any) => t.slug === 'agmarknet');
  ...
  return (
    <div className="min-h-screen bg-neutral50">
      ...
      {/* Data Available */}
      {term.dataAvailable && term.dataAvailable.length > 0 && (
        <section className="py-16">
          <div className="container mx-auto px-4 max-w-6xl">
            <h2 className="text-3xl font-bold text-neutral900 mb-8">
              Data Available on AGMARKNET
            </h2>
            <div className="bg-white rounded-xl p-8 shadow-sm">
              <ul className="grid md:grid-cols-2 gap-4">
                {term.dataAvailable.map((data: string, index: number) => (
                  <li key={index} className="flex items-center gap-3">
                    <span className="text-brandGreen600 text-xl">✓</span>
                    <span className="text-neutral700">{data}</span>
                  </li>
                ))}
              </ul>
            </div>
      ...
      {/* Limitations */}
      {term.limitations && term.limitations.length > 0 && (
        <section className="py-16 bg-white">
          <div className="container mx-auto px-4 max-w-6xl">
            <h2 className="text-3xl font-bold text-neutral900 mb-8">
              Limitations
            ...
            <div className="bg-red50 rounded-xl p-8 border border-red200">
              <ul className="space-y-3">
                {term.limitations.map((limitation: string, index: number) => (
                  <li key={index} className="flex items-start gap-3">
                    <span className="text-red600 text-xl">✗</span>
                    <span className="text-red900">{limitation}</span>
                  </li>
                ))}
              </ul>
            </div>
      ...
      <section className="py-16 bg-brandGreen700 text-white">
        <div className="container mx-auto px-4 max-w-4xl text-center">
          <h2 className="text-3xl font-bold mb-4">
            Get AI-Powered Insights Beyond AGMARKNET
          </h2>
          <p className="text-xl text-white/90 mb-8">
            FlockIQ uses AGMARKNET data to train prediction models and provide 7-day forecasts with sell signals.
          </p>
          <Link
            href="/signup"
