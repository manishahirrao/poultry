// FlockIQ — Timing Loss Glossary Page
// File: apps/web/app/(marketing)/glossary/timing-loss/page.tsx
...
export const metadata: Metadata = {
  title: 'What is Timing Loss? — Poultry Farming Explained',
  description: 'Timing loss is the money lost by selling at the wrong time—too early or too late. Learn causes, impact, and how to prevent ₹40,000+ losses per batch.',
  keywords: ['timing loss meaning', 'sell timing poultry', 'price timing loss', 'timing loss in Hindi', 'poultry sell timing'],
  openGraph: {
    title: 'What is Timing Loss?',
    description: 'Understand timing loss in poultry farming - causes, impact, and prevention strategies.',
    url: 'https://FlockIQ.ai/glossary/timing-loss',
  },
  alternates: {
    canonical: 'https://FlockIQ.ai/glossary/timing-loss',
  },
};

export default function TimingLossPage() {
  const terms = glossaryData.terms;
  const term = terms.find((t: any) => t.slug === 'timing-loss');
  ...
  return (
    <div className="min-h-screen bg-neutral50">
          ...
          <div className="container mx-auto px-4 max-w-6xl">
            ...
            <div className="bg-red50 rounded-xl p-8 border border-red200">
              <ul className="space-y-3">
                {term.causes.map((cause: string, index: number) => (
                  <li key={index} className="flex items-start gap-3">
                    <span className="text-red600 text-xl">✗</span>
                    <span className="text-red900">{cause}</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div className="container mx-auto px-4 max-w-6xl">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <div className="bg-red50 rounded-xl p-6 border border-red200">
                <h3 className="text-lg font-bold text-red900 mb-2">Per Kg Loss</h3>
                <p className="text-3xl font-bold text-red700">{term.impact.perKg}</p>
              </div>
              <div className="bg-red50 rounded-xl p-6 border border-red200">
                <h3 className="text-lg font-bold text-red900 mb-2">Per 20K Birds</h3>
                <p className="text-3xl font-bold text-red700">{term.impact.per20000Birds}</p>
              </div>
              <div className="bg-red50 rounded-xl p-6 border border-red200">
                <h3 className="text-lg font-bold text-red900 mb-2">Per Year (4 Batches)</h3>
                <p className="text-3xl font-bold text-red700">{term.impact.perYear4Batches}</p>
              </div>
            </div>
          </div>
        </section>
      ...
      {/* Solutions */}
      {term.solutions && term.solutions.length > 0 && (
        <section className="py-16">
          <div className="container mx-auto px-4 max-w-6xl">
            <h2 className="text-3xl font-bold text-neutral900 mb-8">
              How to Prevent Timing Loss
            </h2>
            <div className="bg-brandGreen50 rounded-xl p-8 border border-brandGreen200">
              <ul className="space-y-3">
                {term.solutions.map((solution: string, index: number) => (
                  <li key={index} className="flex items-start gap-3">
                    <span className="text-brandGreen600 text-xl">✓</span>
                    <span className="text-brandGreen900">{solution}</span>
                  </li>
                ))}
              </ul>
            </div>
      ...
      <section className="py-16 bg-brandGreen700 text-white">
        <div className="container mx-auto px-4 max-w-4xl text-center">
          <h2 className="text-3xl font-bold mb-4">
            Eliminate Timing Loss with FlockIQ
          </h2>
          <p className="text-xl text-white/90 mb-8">
            Our platform provides 7-day price forecasts and daily sell signals to help you sell at the right time.
          </p>
          <Link
            href="/signup"
