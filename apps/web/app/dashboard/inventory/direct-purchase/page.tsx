...
import { 
  ...
  Download, Printer, Package, Spinner, Truck
      ...
      <div className="flex items-start justify-between gap-8 px-6 pt-8 pb-6">
        <div className="space-y-3">
          ...
          <h1 className="text-3xl font-bold text-[#111827] leading-tight tracking-tight">
            {isHindi ? 'सीधी खरीद' : 'Direct Purchase'}
          </h1>
          <p className="text-base text-[#6B7280] leading-relaxed max-w-lg">
            {isHindi 
              ? 'बिना PO के सीधे खरीद करें - चालान, चालान तिथि, और आइटम विवरण दर्ज करें' 
              : 'Purchase directly without PO - enter invoice, invoice date, and item details'}
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-8">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-[#111827] uppercase tracking-wide">{isHindi ? 'आइटम' : 'Items'}</h3>
                <button
                  type="button"
                  onClick={addLineItem}
              ...
              {lineItems.length === 0 ? (
                <div className="text-center py-12 text-[#6B7280] border-2 border-dashed border-[#E3EDE7] rounded-lg bg-[#EDF7F1]/30">
                  ...
                  <p className="text-sm">{isHindi ? 'कोई आइटम नहीं जोड़ा गया' : 'No items added yet'}</p>
                  <p className="text-xs mt-1">{isHindi ? 'आइटम जोड़ने के लिए बटन पर क्लिक करें' : 'Click the button above to add items'}</p>
                </div>
              ) : (
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-[#EDF7F1] text-[#1A5C34]">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-left font-semibold text-xs uppercase tracking-wider">{isHindi ? 'समाप्ति' : 'Expiry'}</th>
