...
export async function GET(request: NextRequest) {
  try {
    ...
    // Build feed consumption data
    const feedConsumptionData: any[] = [];
    ...
    // Aggregate feed consumption per farm per day
    farms.forEach((farm: any) => {
      const farmName = farm.name.substring(0, 15);
      const birdsAlive = farm.active_batch?.[0]?.birds_alive || 1;
      ...
      logs.forEach((log: any) => {
        ...
        if (dateMap.has(logDate)) {
          ...
          if (log.feed_consumed_kg) {
            dayData[farmName] = log.feed_consumed_kg;
            // Calculate efficiency (gm/bird/day)
            dayData.efficiency = (log.feed_consumed_kg * 1000) / birdsAlive;
          }
        }
      });
    });
    ...
    console.error('Error in feed consumption API:', error);
